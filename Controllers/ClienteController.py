...
class ClienteController:
    ...
    def actualizar_cliente(self, id, nuevo_id=None, nuevo_nombre=None, nuevo_apellido=None):
        ...
        if nuevo_id is None:
            nuevo_id = input("Ingrese el nuevo ID del cliente (o presione Enter para mantener el actual): ").strip()
            nuevo_id = int(nuevo_id) if nuevo_id.isdigit() else None

        if nuevo_nombre is None:
            nuevo_nombre = input("Ingrese el nuevo nombre del cliente (o presione Enter para mantener el actual): ").strip()
            if nuevo_nombre and nuevo_nombre.isdigit():
                print("El nombre no puede contener números.")
                return
            
        if nuevo_apellido is None:
            nuevo_apellido = input("Ingrese el nuevo apellido del cliente (o presione Enter para mantener el actual): ").strip()
            if nuevo_apellido and nuevo_apellido.isdigit():
